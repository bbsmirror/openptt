...
//      resetutmpent();
	...
	getdata(2, 0, "­n©M¥L(¦o) (T)½Í¤Ñ"
		"(N)¨S¨Æ§ä¿ù¤H¤F?[N] ", genbuf, 4, LCECHO);
	switch (*genbuf)
	{
	case 'y':
	case 't':
...
//                  if((diff > IDLE_TIMEOUT + 10) &&
	...
	if (state >= US_ACTION)
	{
	    showtitle((cuser.uflag & FRIEND_FLAG) ? "¦n¤Í¦Cªí" : "¥ð¶¢²á¤Ñ",
		      BBSName);
	    prints("  ¤W¯¸¤H¼Æ¡G%-4d\n"
		   "\033[7m  No.   ¥N¸¹         %-17s%-17s%-13s%-10s\033[m\n",
		   actor, "¼ÊºÙ", "¬G¶m", "°ÊºA", "³Æµù  µo§b");
	}
	else
	{
	    move(3, 0);
