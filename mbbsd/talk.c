...
int my_write(pid_t pid, char *hint, char *id) {
    ...
    {
	outmsg("\033[1;33;41mÁV¿|! ¹ï¤è¤w¸¨¶]¤F(¤£¦b¯¸¤W)! "
	       "\033[37m~>_<~\033[m");
	clrtoeol();
	refresh();
	watermode = -1;
	...
	if ((fp = fopen(genbuf, "a")))
	{
	    fprintf(fp, "\033[1;33;46m¡¹ %s %s\033[37;45m %s \033[0m[%s]\n",
		    cuser.userid, (*hint == 2) ? "\033[33;41m¼s¼½" : "",
		    msg, Cdatelite(&now));
	    fclose(fp);
	}
	...
	if (uin->msgcount < MAX_MSGS)
	{
	    ...
	    outmsg("\033[1;33;41mÁV¿|! ¹ï¤è¤£¦æ¤F! (¦¬¨ì¤Ó¦h¤ô²y) "
		   "\033[37m@_@\033[m");
...
//Ptt:´î¤Ösort¦¸¼Æ state = US_PICKUP; 
	...
	while (!state)
	{
		...
		if (cuser.uflag & FRIEND_FLAG || HAS_PERM(PERM_SYSOP))
		{
		    ...
		    state = US_PICKUP;
		    if (!getdata(0, 0, "¼s¼½°T®§:", genbuf + 1, 60, DOECHO))
			break;
